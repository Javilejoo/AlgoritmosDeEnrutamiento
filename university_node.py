...
class DistributedNode:
    """Nodo que implementa el protocolo universitario"""
    ...
    def __init__(self, node_id: str, port: int, algorithm: str = "flooding"):
        ...
        print(f"🎓 Nodo Universitario {node_id} creado (Algoritmo: {algorithm})")
    
    async def start(self):
        """Inicia el nodo universitario"""
        self.running = True
    ...
    def add_neighbour(self, neighbor_id: str, cost: int, ip: str = "localhost", port: int = None):
        """Agrega un vecino según protocolo universitario"""
        self.neighbours[neighbor_id] = cost
        if port:
            self.neighbor_connections[neighbor_id] = (ip, port)
    ...
    async def send_done_message(self):
        ...
        done_msg = UniversityMessageFactory.create_done_message(self.node_id)
        
        await self._broadcast_to_neighbours(done_msg)
        self.calculation_done = True
        print(f"✅ Mensaje DONE enviado desde {self.node_id}")
    
    async def _broadcast_to_neighbours(self, message: UniversityMessage):
    ...
    async def _send_to_node(self, node_id: str, message: UniversityMessage):
    ...
    async def _process_message(self, message_str: str, from_addr):
        ...
        try:
            message = UniversityMessage.from_json(message_str)
            
            if not UniversityProtocolValidator.validate_university_message(message):
    ...
    async def _handle_init_message(self, message: UniversityMessage):
        """Maneja mensaje INIT"""
        data = message.data
    ...
    async def _handle_user_message(self, message: UniversityMessage):
    ...
    async def _handle_done_message(self, message: UniversityMessage):
        """Maneja mensaje DONE"""
        neighbor_id = message.data["whoAmI"]
        self.done_messages_received += 1
        print(f"✅ {neighbor_id} terminó sus cálculos")
    
    async def _flood_message(self, message: UniversityMessage):
    ...
    async def _flood_user_message(self, message: UniversityMessage, msg_id: str):
        ...
        updated_message = UniversityMessage(
            type="message",
            data=message.data
        )
    ...
    async def _route_message(self, message: UniversityMessage):
    ...
    async def _route_user_message(self, message: UniversityMessage):
...
# Demo del nodo universitario
async def demo_university_nodes():
    """Demo de nodos universitarios"""
    print("=== DEMO NODOS UNIVERSITARIOS ===\n")
    
    # Crear nodos
    node_a = UniversityNode("A", 65401, "flooding")
    node_b = UniversityNode("B", 65402, "lsr")
    node_c = UniversityNode("C", 65403, "flooding")
...
if __name__ == "__main__":
    asyncio.run(demo_university_nodes())
